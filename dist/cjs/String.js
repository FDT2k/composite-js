...
var toSnake = either(isCapitalLetter, identity, compose(concat('_'), lcase));

var snakeReducer = function snakeReducer(acc, item, idx) {
  var letter = either(function (_) {
    return idx === 0;
  }, toSnake, lcase)(item);
  acc.push(letter);
  return acc;
...
var camelMapper = function camelMapper(item, idx) {
  return either(function (_) {
    return idx === 0;
  }, ucfirst, identity)(item);
};
var camelToSnakeCase = compose(joinList(''), function (x) {
  return reduce([], snakeReducer, x);
}, split(''));
var snakeToCamelCase = compose(joinList(''), map(camelMapper), split('_'));
var snakeToUpperCamelCase = compose(snakeToCamelCase, ucfirst);
var snakeToLowerCamelCase = compose(snakeToCamelCase, lcfirst);
var matchRuleEscapeRegex = replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "\\$1");
var matchStrToRegex = function matchStrToRegex(escapeRegex) {
  return compose(regex, concat('^'), append('$'), joinList(".*"), map(escapeRegex), split("*"));
};
var matchRule = curry(function (rule, str) {
  return test(matchStrToRegex(matchRuleEscapeRegex)(rule))(str);
});

exports.camelMapper = camelMapper;
exports.camelToSnakeCase = camelToSnakeCase;
exports.matchRule = matchRule;
exports.matchRuleEscapeRegex = matchRuleEscapeRegex;
exports.matchStrToRegex = matchStrToRegex;
exports.snakeToCamelCase = snakeToCamelCase;
exports.snakeToLowerCamelCase = snakeToLowerCamelCase;
exports.snakeToUpperCamelCase = snakeToUpperCamelCase;
