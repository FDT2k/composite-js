...
var delFromList = curry(function (list, val) {
  return filter(isStrictlyNotEqual(val), list);
});
...
var addToListUniq = curry(function (list, val) {
  return add(del(list, val), val);
});
var updateList = curry(function (cond, val, list, fn) {
  return map(either(cond(val), identity, fn))(list);
});
var updateListIfPropEqual = curry(function (prop, val, list, fn) {
  return updateList(isPropStrictlyEqual(prop), val, list, fn);
});
var delFromObjectByKey = curry(function (string, obj) {
  return filterByKey(isStrictlyNotEqual(string))(obj);
}); // String -> Object -> ? -> Object
